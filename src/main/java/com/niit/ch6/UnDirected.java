...
public class UnDirected {
    public static void main(String[] args) throws Exception {
        ...
        // Edges: Undirectional relationships representing "follows"
        ...
        // For undirected graph, we need to add both directions for each edge
        DataSet<Edge<Long, Double>> undirectedEdges = edges
                .union(edges.map(new MapFunction<Edge<Long, Double>, Edge<Long, Double>>() {
                    ...
                    public Edge<Long, Double> map(Edge<Long, Double> edge) throws Exception {
                        return new Edge<>(edge.getTarget(), edge.getSource(), edge.getValue()); // Reverse the edge direction
                    }
                }));  // Reverse the edge direction

        // Create the graph from updated edges (undirected)
